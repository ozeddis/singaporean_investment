...
<?php
	if(isset($_POST['save_btn'])){
		...
		$bonus_type = htmlentities(trim(mysqli_real_escape_string($connection, $_POST['type'])));
			...
			while($result = mysqli_fetch_array($run_query)){
				...
				if(!is_numeric($amount)){
					...
					if($run_query1 == TRUE){
						...
						alert(\"Bonus allocated\");
						window.location = \"admin_dashboard.php?p=allocate_bonus\"
						</script>";
					}else{
						$message_failure = "Failed to allocate `BONUS!!` ";
					}
				}elseif($bonus_type == "roi"){
					$roi = "ROI";
					$query2 = "INSERT INTO wallet (account_id, wallet_address,amount,status,transaction_id,date,time, description) VALUES ('{$account_id}','{$wallet}','{$amount}','{$status}','{$transaction}','{$date}','{$time2}', '{$roi}')";
					...
					if($run_query2 == TRUE){
						...
						$message_failure = "Failed to allocate `ROI!!` ";
					}
				}elseif($bonus_type == "ref"){
					$query1 = "INSERT INTO wallet (account_id, wallet_address,amount,status,transaction_id,date,time) VALUES ('{$account_id}','{$wallet}','{$amount}','{$status}','{$transaction}','{$date}','{$time2}')";
					$run_query1 = mysqli_query($connection, $query1);
					...
					$message_failure = "Select destination";
				}
			}
		}else{
...
<div class="page-title">
  <div>
	<h1><i class="fa fa-archive"></i> Bonus Allocation</h1>
  </div>
  <div>
	<ul class="breadcrumb">
				  ...
				  <div class="card-body">
					<form class="form-horizontal" action="" method="POST">
					  <div class="form-group">
						...
						<div class="col-md-8">
						  <input class="form-control" type="text" name="amount" value="" placeholder="Credit amount" >
						</div>

						<label class="control-label col-md-4">Select destination</label>
						<div class="col-md-8">
							...
							<div class="row">
							  <div class="col-md-8 col-md-offset-3">
								<button class="btn btn-primary icon-btn" type="submit" name="save_btn">
									<i class="fa fa-fw fa-lg fa-check-circle"></i>Validate</button>
							  </div>
							</div>
						  </div>
						</div>

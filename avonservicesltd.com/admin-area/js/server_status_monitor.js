...
AJAX.registerOnload('server_status_monitor.js', function () {
    ...
    $('a[href="#addNewChart"]').click(function (event) {
        ...
        $('#seriesPreview').php('<i>' + PMA_messages.strNone + '</i>');
    ...
    $('a[href="#importMonitorConfig"]').click(function (event) {
        ...
        $('#emptyDialog').php(PMA_messages.strImportDialogMessage + ':<br/><form>' +
    ...
    $('a[href="#pauseCharts"]').click(function (event) {
        ...
        if (! runtime.redrawCharts) {
            $(this).php(PMA_getImage('play') + PMA_messages.strResumeMonitor);
        } else {
            $(this).php(PMA_getImage('pause') + PMA_messages.strPauseMonitor);
            if (! runtime.charts) {
                initGrid();
        ...
        var loadLogVars = function (getvars) {
            ...
            $.post('server_status_monitor.php' + PMA_commonParams.get('common_query'), vars,
                function (data) {
                    ...
                    $dialog.find('div.ajaxContent').php(str);
    ...
    function initGrid () {
        ...
        if (isStorageSupported('localStorage')) {
            ...
            if (runtime.charts !== null
                ...
                $('#emptyDialog').php(PMA_messages.strIncompatibleMonitorConfigDescription);
        ...
        $('#chartGrid').php('<tr><td></td><td></td></tr><tr><td></td><td></td></tr>');
        chartSpacing = {
    ...
    function loadLogStatistics (opts) {
        ...
        $('#emptyDialog').php(PMA_messages.strAnalysingLogs +
        ...
        logRequest = $.post(
            ...
            function (data) {
                ...
                if (logData.rows.length === 0) {
                    ...
                    $('#emptyDialog').php('<p>' + PMA_messages.strNoDataFound + '</p>');
                ...
                $('#emptyDialog').php('<p>' + PMA_messages.strLogDataLoaded + '</p>');
        ...
        */
        function filterQueries (varFilterChange) {
            ...
            $('#logTable').find('table tbody tr td:nth-child(' + (runtime.logDataCols.length - 1) + ')').each(function () {
                ...
                if (varFilterChange && $t.php().match(/^SELECT/i)) {
                    if (noVars) {
            ...
            $('#logTable').find('table tfoot tr')
                .php('<th colspan="' + (runtime.logDataCols.length - 1) + '">' +
    ...
    function openQueryAnalyzer () {
        ...
        $('#queryAnalyzerDialog').dialog({
            ...
            close: function () {
                ...
                $('#queryAnalyzerDialog').find('div.placeHolder').php('');
                if (codemirror_editor) {
    ...
    function loadQueryAnalysis (rowData) {
        ...
        $.post('server_status_monitor.php' + PMA_commonParams.get('common_query'), {
            ...
            if (data.error) {
                ...
                $('#queryAnalyzerDialog').find('div.placeHolder').php('<div class="error">' + data.error + '</div>');
                return;
            }
            var totalTime = 0;
            // Float sux, I'll use table :(
            $('#queryAnalyzerDialog').find('div.placeHolder')
                .php('<table width="100%" border="0"><tr><td class="explain"></td><td class="chart"></td></tr></table>');

            var explain = '<b>' + PMA_messages.strExplainOutput + '</b> ' + $('#explain_docu').php();
            ...
            if (data.profiling) {
                ...
                $('#queryAnalyzerDialog').find('div.placeHolder td.chart').append(
                    '<b>' + PMA_messages.strProfilingResults + ' ' + $('#profiling_docu').php() + '</b> ' +

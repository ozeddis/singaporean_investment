    ...
    $.post(
        ...
        function (data) {
            if (data.success === true) {
                $('select[name=makeAtomic]').php(data.message);
            }
        }
    );
}
function goTo3NFStep1 (newTables) {
    ...
    $.post(
        'normalization.php',
        {
            ...
            $('#page_content').find('h3').php(PMA_messages.str3NFNormalization);
            $('#mainContent').find('legend').php(data.legendText);
            $('#mainContent').find('h4').php(data.headText);
            $('#mainContent').find('p').php(data.subText);
            $('#mainContent').find('#extra').php(data.extra);
...
function goTo2NFStep1 () {
    $.post(
        'normalization.php',
        {
            ...
            $('#page_content h3').php(PMA_messages.str2NFNormalization);
            $('#mainContent legend').php(data.legendText);
            $('#mainContent h4').php(data.headText);
            $('#mainContent p').php(data.subText);
            $('#mainContent #extra').php(data.extra);
            $('#mainContent #newCols').php('');
            if (data.subText !== '') {
                ...
                if (normalizeto === '3nf') {
                    $('#mainContent #newCols').php(PMA_messages.strToNextStep);
                    setTimeout(function () {
...
function goToStep2 (extra) {
    $.post(
        'normalization.php',
        {
            ...
            $('#mainContent h4').php(data.headText);
            $('#mainContent p').php(data.subText);
            $('#mainContent #extra,#mainContent #newCols').php('');
            $('.tblFooters').php('');
            if (data.hasPrimaryKey === '1') {
                if (extra === 'goToStep3') {
                    $('#mainContent h4').php(PMA_messages.strPrimaryKeyAdded);
                    $('#mainContent p').php(PMA_messages.strToNextStep);
                }
...
function goTo2NFStep2 (pd, primary_key) {
    $('#newCols').php('');
    $('#mainContent legend').php(PMA_messages.strStep + ' 2.2 ' + PMA_messages.strConfirmPd);
    $('#mainContent h4').php(PMA_messages.strSelectedPd);
    $('#mainContent p').php(PMA_messages.strPdHintNote);
    ...
    $('#mainContent #extra').php(extra);
    $('.tblFooters').php('<input type="button" value="' + PMA_messages.strBack + '" id="backEditPd"/><input type="button" id="goTo2NFFinish" value="' + PMA_messages.strGo + '"/>');
...
function goTo3NFStep2 (pd, tablesTds) {
    $('#newCols').php('');
    $('#mainContent legend').php(PMA_messages.strStep + ' 3.2 ' + PMA_messages.strConfirmTd);
    $('#mainContent h4').php(PMA_messages.strSelectedTd);
    $('#mainContent p').php(PMA_messages.strPdHintNote);
    var extra = '<div class="dependencies_box">';
    var pdFound = false;
    ...
    $('#mainContent #extra').php(extra);
    $('.tblFooters').php('<input type="button" value="' + PMA_messages.strBack + '" id="backEditPd"/><input type="button" id="goTo3NFFinish" value="' + PMA_messages.strGo + '"/>');
...
AJAX.registerOnload('normalization.js', function () {
    ...
    $('#mainContent').on('click', '#backEditPd', function () {
        $('#mainContent').php(backup);
    });
    $('#mainContent').on('click', '#showPossiblePd', function () {
        if ($(this).hasClass('hideList')) {
            $(this).php('+ ' + PMA_messages.strShowPossiblePd);
            $(this).removeClass('hideList');
            $('#newCols').slideToggle('slow');
            return false;
        }
        if ($('#newCols').php() !== '') {
            $('#showPossiblePd').php('- ' + PMA_messages.strHidePd);
            $('#showPossiblePd').addClass('hideList');
            $('#newCols').slideToggle('slow');
            return false;
        }
        $('#newCols').insertAfter('#mainContent h4');
        $('#newCols').php('<div class="center">' + PMA_messages.strLoading + '<br/>' + PMA_messages.strWaitForPd + '</div>');
        $.post(
            'normalization.php',
            {
                'ajax_request': true,
    ...
    $('#mainContent').on('click', '.pickPd', function () {
        var strColsLeft = $(this).next('.determinants').php();
        var colsLeft = strColsLeft.split(',');
        var strColsRight = $(this).next().next().php();

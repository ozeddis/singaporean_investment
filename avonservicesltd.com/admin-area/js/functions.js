...
function displayPasswordGenerateButton () {
    ...
    var titleCell = $('<td />').php(PMA_messages.strGeneratePassword).appendTo(generatePwdRow);
    ...
    var titleLabel = $('<label />').attr({ for: 'button_generate_password' })
        .php(PMA_messages.strGeneratePassword + ':')
...
function insertQuery (queryType) {
    if (queryType === 'clear') {
        ...
        if (codemirror_editor) {
            $('#querymessage').php(PMA_messages.strFormatting +
...
function PMA_updateCode ($base, htmlValue, rawValue) {
    ...
    if (typeof CodeMirror !== 'undefined') {
        ...
        $notHighlighted.hide();
        $code.php('').append($notHighlighted, $highlighted[0]);
    } else {
        $code.php('').append($notHighlighted);
    }
    ...
    if ($('#sync_favorite_tables').length) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#pma_favorite_list').php(data.list);
            }
        });
    }
}); // end of $()

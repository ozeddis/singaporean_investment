...
AJAX.registerOnload('db_central_columns.js', function () {
    ...
    $('.edit').click(function () {
        ...
        var attribute_val = $('#f_' + rownum + ' td[name=col_attribute] span').php();
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.message !== '1') {
                    ...
                    $('#f_' + rownum + ' td input[id=checkbox_row_' + rownum + ']').val($('#f_' + rownum + ' input[name=col_name]').val()).php();
                    $('#f_' + rownum + ' td[name=col_name] span').text($('#f_' + rownum + ' input[name=col_name]').val()).php();
                    $('#f_' + rownum + ' td[name=col_type] span').text($('#f_' + rownum + ' select[name=col_type]').val()).php();
                    $('#f_' + rownum + ' td[name=col_length] span').text($('#f_' + rownum + ' input[name=col_length]').val()).php();
                    $('#f_' + rownum + ' td[name=collation] span').text($('#f_' + rownum + ' select[name=collation]').val()).php();
                    $('#f_' + rownum + ' td[name=col_attribute] span').text($('#f_' + rownum + ' select[name=col_attribute]').val()).php();
                    $('#f_' + rownum + ' td[name=col_isNull] span').text($('#f_' + rownum + ' input[name=col_isNull]').is(':checked') ? 'Yes' : 'No').php();
                    $('#f_' + rownum + ' td[name=col_extra] span').text($('#f_' + rownum + ' input[name=col_extra]').is(':checked') ? 'auto_increment' : '').php();
                    $('#f_' + rownum + ' td[name=col_default] span').text($('#f_' + rownum + ' :input[name=col_default]').val()).php();
                }
                $('#save_' + rownum).hide();
                $('#edit_' + rownum).show();
                $('#f_' + rownum + ' td span').show();
    ...
    $('#table-select').change(function (e) {
        ...
        $('#column-select').php('<option value="">' + PMA_messages.strLoading + '</option>');
        if (selectvalue !== '') {
    ...
    $('#add_col_div').find('>a').click(function (event) {
        ...
        if ($addColDivLinkSpan.php() === '+') {
            $addColDivLinkSpan.php('-');
        } else {
            $addColDivLinkSpan.php('+');
        }
    });

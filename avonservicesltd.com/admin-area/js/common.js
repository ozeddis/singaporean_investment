...
PMA_DROP_IMPORT = {
        ...
        $('.pma_sql_import_status div li[data-hash="' + hash +
            ...
            .on('click', function () {
                if ($(this).attr('task') === 'cancel') {
                    jqXHR.abort();
                    $(this).php('<span>' + PMA_messages.dropImportMessageAborted + '</span>');
                    PMA_DROP_IMPORT._importFinished(hash, true, false);
                } else if ($(this).children('span').php() ===
    ...
    _dragenter : function (event) {
        ...
        if (PMA_commonParams.get('db') === '') {
            $('.pma_drop_handler').php(PMA_messages.dropImportSelectDB);
        } else {
            $('.pma_drop_handler').php(PMA_messages.dropImportDropFiles);
        }
    ...
    _importFinished: function (hash, aborted, status) {
        ...
        if (!aborted) {
            if (status) {
                $('.pma_sql_import_status div li[data-hash="' + hash +
                   '"] span.filesize span.pma_drop_file_status')
                    .php('<span>' + PMA_messages.dropImportMessageSuccess + '</a>');
            } else {
                $('.pma_sql_import_status div li[data-hash="' + hash +
                   '"] span.filesize span.pma_drop_file_status')
                    .php('<span class="underline">' + PMA_messages.dropImportMessageFailed +
                   '</a>');
        ...
        $('.pma_import_count').php(--PMA_DROP_IMPORT.liveUploadCount);

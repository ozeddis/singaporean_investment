        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.changes) {
                    ...
                    $('#' + anchor_id).parent().php(data.anchor);
                    PMA_tooltip(
                        $('#' + anchor_id),
                        'a',
    ...
    if (!$('#pma_navigation_settings').length) {
        ...
        $.post(url, params, function (data) {
            if (typeof data !== 'undefined' && data.success) {
                $('#pma_navi_settings_container').php(data.message);
                setupRestoreField();
                setupValidation();
                setupConfigTabs();
...
function PMA_reloadNavigation (callback, paths) {
    ...
    function requestNaviReload (params) {
        ...
        $.post(url, params, function (data) {
            if (typeof data !== 'undefined' && data.success) {
                $('#pma_navigation_tree').php(data.message).children('div').show();
    ...
    $.post(url, params, function (data) {
        if (typeof data !== 'undefined' && data.success) {
            ...
            if (isDbSelector) {
                ...
                $this.closest('div.list_container').php(
...
var ResizeHandler = function () {
    ...
    $('#pma_navigation_collapser').php(this.getSymbol($('#pma_navigation').width()));
...
PMA_fastFilter.filter.prototype.restore = function (focus) {
    if (this.$this.children('ul').first().hasClass('search_results')) {
        this.$this.php(this.$clone.php()).children().show();
        this.$this.data('fastFilter', this);
        if (focus) {
